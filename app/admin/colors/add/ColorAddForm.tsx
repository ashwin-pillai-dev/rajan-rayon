...
import { addColor, updateColor } from '../actions';
import { colorsFormType, colorSchema } from './colorSchema';

interface colorProps {
  isEdit: boolean;
  colorId?: number;
  colorData?: colorsFormType;
}

export default function ColorAddForm({ isEdit, colorId, colorData }: colorProps) {
  const {
    register,
    handleSubmit,
    getValues,
    formState: { errors, isSubmitting },
  } = useForm<colorsFormType>({
    defaultValues: isEdit ? colorData : {},
  ...
  async function onsubmit(data: colorsFormType) {
    try {
      console.log('color data: ', data);

      if (isEdit) {
        if (colorId) {
          const res = await updateColor(data, colorId);
          if (res) {
            succesToastMessage({ message: 'color updated successfully' });
        ...
        if (res) {
          succesToastMessage({ message: 'color added successfully' });
          router.push('/admin/colors/list');
        }
      }
    } catch (error) {
      failToastMessage({ message: 'Failed adding color' });
    }
  }
  ...
  return (
    <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit(onsubmit)}>
      ...
      <Button size="xs" type="submit" disabled={isSubmitting} className="w-full bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 rounded-lg px-5 py-2.5">
        <p className="text-white font-medium text-sm">{isEdit ? 'Edit Color' : 'Add Color'}</p>
      </Button>
    </form>
  );
}
